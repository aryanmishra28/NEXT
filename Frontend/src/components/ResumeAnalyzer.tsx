...
export function ResumeAnalyzer() {
  ...
  // Helper to normalize fields from backend
  const normalizedScore = React.useMemo(() => {
    const s = (analysisData?.ats_score ?? analysisData?.ats_score_estimate ?? analysisData?.score);
    return typeof s === 'number' ? Math.max(0, Math.min(100, s)) : 0;
  }, [analysisData]);

  const normalizedSkills: string[] = React.useMemo(() => {
    if (!analysisData || !analysisData.top_skills) return [];
    // backend may return object {technical, soft, tools} or a flat array
    const ts = analysisData.top_skills;
    if (Array.isArray(ts)) return ts.filter(Boolean);
    const technical = Array.isArray(ts.technical) ? ts.technical : [];
    const soft = Array.isArray(ts.soft) ? ts.soft : [];
    const tools = Array.isArray(ts.tools) ? ts.tools : [];
            ...
            {analysisData && (
              <>
                <Card className="shadow-lg rounded-2xl">
                  ...
                  <CardContent>
                    <Progress value={normalizedScore} className="h-3 mb-4" />
                    ...
                    <CardContent className="space-y-4">
                      {normalizedSkills.length > 0 ? (
                        <div className="flex flex-wrap gap-2">
                          {normalizedSkills.map((skill: string, idx: number) => (
