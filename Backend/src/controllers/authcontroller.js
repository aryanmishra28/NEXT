...
const register = async (req, res) => {
  try {
    ...
    // Create user with initial welcome activity
    const user = await User.create({
      ...
      recentActivity: [
        {
          type: 'welcome',
          message: `Welcome to NEXT STEP, ${name}! 🎉`,
          time: 'Just now',
          createdAt: new Date()
        },
        {
...
// Function to add activity to user
const updateActivity = async (req, res) => {
  try {
    ...
    const { type = 'general', message, time } = req.body;
    ...
    // Add new activity (limit to last 5 activities)
    const newActivity = {
      type,
      message,
